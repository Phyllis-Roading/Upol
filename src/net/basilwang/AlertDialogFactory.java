...
import static net.basilwang.dao.Preferences.*;

public class AlertDialogFactory {
  
	 public static Builder getYoumiOfferDialog(final Activity activity,final CheckBoxPreference checkBoxPreference)
	 {
		 String prefix = null;
		 ...
		 checkBoxPreference.setChecked(false);
			...
			title.setText(prefix+"需要30积分，您现在有"
					+ YoumiPointsManager
							.queryPoints(activity)
			...
			builder.setNegativeButton(prefix,
					new OnClickListener() {
						@Override
						public void onClick(DialogInterface dialog,
								int which) {
							...
							if (score >= 30) {
								if (YoumiPointsManager.spendPoints(
									...
									SharedPreferences.Editor ed = Preferences
											.getEditor(activity);
									//2013-01-23 basilwang fix a severity bug in 2.0 that can't close ad cause I don't save the status at all, damn it
									 if(checkBoxPreferenceKey.equals(WEEKVIEW_ENABLED))
									 {
									     ed.putBoolean(
